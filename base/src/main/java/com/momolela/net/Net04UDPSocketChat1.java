...
public class Net04UDPSocketChat1 {

    public static void main(String[] args) throws Exception {
        DatagramSocket datagramSocketSend = new DatagramSocket();
        DatagramSocket datagramSocketReceive = new DatagramSocket(10000);
        new Thread(new Send1(datagramSocketSend)).start();
        new Thread(new Receive1(datagramSocketReceive)).start();
    }
...
class Send1 implements Runnable {
    ...
    public Send1(DatagramSocket datagramSocket) {
    ...
    public void run() {
        try {
            ...
            while ((line = bufferedReader.readLine()) != null) {
                ...
                DatagramPacket datagramPacket = new DatagramPacket(bytes, bytes.length, InetAddress.getByName("127.0.0.1"), 10001); // 这里有个东西实现群聊，xxx.xxx.xxx.0（代表网络段地址），xxx.xxx.xxx.255（这个网络段的广播地址） 是两个特殊地址，用广播地址，可以给这个网络段的所有机器发信息
...
class Receive1 implements Runnable {
    ...
    public Receive1(DatagramSocket datagramSocket) {
        this.datagramSocket = datagramSocket;
    }
