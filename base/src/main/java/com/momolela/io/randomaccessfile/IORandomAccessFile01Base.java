...
/**
 * RandomAccessFile 不属于 IO 体系，但是在 IO 包中成员，因为他包含读和写的功能，内部就是用字节流实现，只有流才能操作数据
 * 内部封装了一个数组，通过指针操作数组元素，可以通过 .getFilePointer() 方法可以获取指针位置，也可以通过 .seek() 改变指针位置
 * 通过构造函数，这个类只能操作文件，而且 mode 可以有 r rw ... 等模式
...
//        String name = new String(randomAccessFile.readLine().getBytes("ISO-8859-1"), "utf-8"); // readLine() 需要转码
